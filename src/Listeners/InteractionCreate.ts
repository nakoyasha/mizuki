import { ButtonInteraction, CommandInteraction, Interaction, ModalSubmitInteraction, PermissionsBitField } from "discord.js";
...
const handleSlashCommand = async (interaction: CommandInteraction): Promise<void> => {
    ...
    var failedPermissionCheck = false;
    if (!slashCommand) {
    ...
    try {
        if (slashCommand.ownerOnly == true) {
            if (interaction.user.id != process.env.OWNER_ID) {
                interaction.reply({ content: "Command failed to execute: This command is currently owner-only and is thus not available to be ran by anyone else (the description might tell you why its owner-only)", ephemeral: true })
                return;
            }
        }
    } catch (err) {
        logger.error(`Command failed to execute: Failed to send the owner-only warning message ${err}`)
    }
        ...
        if (slashCommand.permissions != undefined) {
            for await (const permissionBit of slashCommand.permissions) {
                if (!interaction.memberPermissions?.has(permissionBit as bigint)) {
                    ...
                    await interaction.reply({
                        content: `Command failed to execute: You are missing the ${getPermissionName(permissionBit)} permission.`
                    })
                    return;
                }
    ...
    if (failedPermissionCheck === true) { return; }
