...
export const JobSystem = {
    ...
    createJob(name: string, task: () => any, onFinish?: onFinish) {
        ...
        console.log(`Job ${job.name} has been added to queue with position #${this.JobQueue.indexOf(job)} `)
    },
        ...
        this.JobQueue.forEach(job => {
            ...
            job.run((status, err) => {
                if (status == JobResult.Failed) {
                    console.log(`Job ${job.name} in position ${activeIndex} has failed with error ${err}`)
                } else {
                    console.log(`Job ${job.name} in position ${activeIndex} has finished successfully`)
                }
                // remove from queue
