...
async function sendOutput(output: string, interaction: CommandInteraction) {
  ...
  if (output.length >= 2000) {
    ...
    await interaction.followUp({
      content:
        "Output has been sent in a file due to it exceeding the 2000 character limit.",
      files: [attachment],
    });
    return;
  }
...
export const Eval: CommandV2 = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName("code").setDescription("The code to run").setRequired(true)
    ),
  ownerOnly: true,
      ...
      runCode()
        ...
        .catch(async (err: Error) => {
          if (err.message.includes("Cannot send an empty message")) {
            await interaction.followUp(
              constants.messages.EVAL_CODE_RAN_NO_OUTPUT
            );
            return;
          }

          let stack = err.stack;
          let paths = stack?.match(/\/home\/.*\//);

          if (paths == undefined) {
            // fallback
            paths = ["/home/nakoyasha"];
          }

          for (let path of paths) {
            stack = stack?.replaceAll(path, "");
          }

          await sendOutput(
            `Error thrown during execution: ${codeblock(
              stack as string,
              "js"
            )}`,
            interaction
