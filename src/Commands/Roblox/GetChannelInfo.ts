...
function resolveChannelLink(BinaryType: any, ChannelName: any) {
    return "https://clientsettings.roblox.com/v2/client-version/" + BinaryType + "/channel/" + ChannelName
}

async function getChannelInfo(BinaryType: any, ChannelName: any) {
    try {
        const response = await axios.get(resolveChannelLink(BinaryType, ChannelName))
